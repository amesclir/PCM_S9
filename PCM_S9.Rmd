...
Para ajustar este tipo de modelos en datos reales usaremos el paquete de R diversichrom (Fitzjohn, 2010).
...
En vez de usar la función make.bd usaremos la función make.bd.t que es similar a la anterior pero incluye un parámetro extra que es la función que usaremos para analizar como cambia la especiación o la extinción en función del tiempo.
...
El valor de lambda.l es el valor de especiación en la raiz de la filogenia y el valor de lambda.a el valor ajustado de la función exponencial (es contraintuitivo que este valor sea positivo, ya que representa una tasa a la cual disminuya la especiación).
...
Aunque sea dificil de ver, la recta que representa la especiación está un poco inclinada hacia abajo.
...
Acuerdate de que cada vez que corras un MCMC bayesiano debes de comprobar la convergencia del modelo tal y como lo hicimos con el paquete coda en la sesión anterior.
...
Por supuesto es posible también ajustar los modelo con MCMC bayesiano.
...
Como era de esperar el meojr modelo es de tasas de especiación y extinción constantes.


MODELOS DE DIVERSIFICACIÓN DEPENDIENTES DE LA RIQUEZA DE LINAJES O ESPECIES

Muchos investigadores han sugerido que las tasas de espeicación y extinción pueden ser dependientes no del tiempo sino del número de linajes (Rabosky and Hurlbert 2015; but see Harmon and Harrison, 2015).
...
Pero modelos dependiente directamente de diversidad han sido también desarrollados more directly (Etienne et al. 2012, 2016;Etienne and Haegeman 2012).
...
Y ahora vamos ajustar el modelo de espeación y extinción constante, los modelos en los que estos varian y también nuestro modelo dependiente de la diversidad de linajes. Los ajustaremos todos con el paquete DDD.
...
Y ahora vamos a ajustar los modelos de especiación y extinción dependientes del tiempo (con el argumento tdmodel=1 le indicamos que especición y/o extinción pueden ser dependientes exponencialmente del tiempo).
...
En este modelo se calcula K que es la capacidad de carga del proceso de especiación (número máximo de especies o linajes que soporta el sistema) y donde la diversificacion es cero, es decir, espeicación y extinción alcanzar el mismo valor.
...
DIVERSIFICACIÓN VARIABLES A LO LARGO DE LOS CLADOS

De nuevo varios autores han sugerido que las tasas de diversificación pueden variar entre clados (Morlon et al., 2011) y curiosamente dentro de los clados pueden después variar en función del tiempo o de las especies (Morlon et al., 2011).
...
Aohra vamos a procesar los datos con el paquete de R BAMMtools.
...
1. Ajusta todos los modelos que hemos aprendido en esta sesión pero con el árbol "Anolis.tre".
